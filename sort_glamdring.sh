...
grep r1 multirun.sh.o* | awk -f ~/VIMOS_project/analysis/awkFile.txt | grep ^r1 | sed 's/r[1-2] *$//' | sed 's/[a-z]* *$//'| sed 's/[A-Z]*$//' > glamdring_result.txt

grep r2 multirun.sh.o* | awk -f ~/VIMOS_project/analysis/awkFile2.txt | grep ^r2 | sed 's/r[1-2] *$//' | sed 's/[a-z]* *$//' | sed 's/[A-Z]*$//' > glamdring_result2.txt
