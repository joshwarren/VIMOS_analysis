...
def fit_disk(galaxy, D=None, opt='kin'):
	...
	disk,pars=dfn.disk_fit_exp(D.xBar,D.yBar,vel.copy(),vel_err.copy(),leeway=leeway, 
		sigclip=sigclip, grid_length=40)
	...
	ax[1,1] = plot_velfield_nointerp(D.x, D.y, D.bin_num, D.xBar, D.yBar, disk, header, 
		#vmin=vmin, vmax=vmax, 
	...
	ax[0,1] = plot_velfield_nointerp(D.x, D.y, D.bin_num, D.xBar, D.yBar, disk, header, 
		#vmin=vmin, vmax=vmax, 
	...
	ax2[2] = plot_velfield_nointerp(D.x, D.y, D.bin_num, D.xBar, D.yBar, plot, header, 
		# vmin=vmin, vmax=vmax, 
		flux_unbinned=D.unbinned_flux, nodots=True, colorbar=True, 
		ax=ax2[2], title=r'Residuals: v$_\mathrm{\ast}$ - v$_\mathrm{\ast,mod}$')

	ax2[0] = plot_velfield_nointerp(D.x, D.y, D.bin_num, D.xBar, D.yBar, 
		D.components['stellar'].plot['vel'].uncert, header, #vmin=vmin, vmax=vmax, 
		flux_unbinned=D.unbinned_flux, nodots=True, colorbar=True, ax=ax2[0],
		title=r'Stellar velocity uncertainty')

	ax2[1] = plot_velfield_nointerp(D.x, D.y, D.bin_num, D.xBar, D.yBar, 
		D.components['[OIII]5007d'].plot['vel'].uncert, header, #vmin=vmin, vmax=vmax, 
		flux_unbinned=D.unbinned_flux, nodots=True, colorbar=True, ax=ax2[1],
		title=r'Gas velocity Uncertainty')

	fig2.savefig('%s/Data/vimos/analysis/%s/%s/plots/outflows2.png' % (cc.base_dir, galaxy,
		opt))
