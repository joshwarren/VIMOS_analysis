...
;; ----------===============================================---------
	...
	moments = [4,2] ; (v,sig,h3,h4) for stars, (v,sig) for gas (defualt)
...
; Finding the template files
	stemplatesDirectory = '/Data/idl_libraries/ppxf/MILES_library/'
	stemplateFiles = FILE_SEARCH(stemplatesDirectory + $
		'm0[0-9][0-9][0-9]V', COUNT=nfiles)

;v1 is wavelength, v2 is spectrum
	READCOL, stemplateFiles[0], v1,v2, FORMAT = 'D,D', /SILENT
...
;	temp_stemplate = MAKE_ARRAY(NAXIS1, nfiles)

	lamRange_stemplate = CRVAL_temp + [0d, CDELT_temp*(NAXIS_temp-1d)]
        log_rebin, lamRange_stemplate, v1, log_temp_stemplate, $
		logLam_stemplate, velscale=velscale
...
;; transfered to the array templatesToUse.
	...
	stemplates = MAKE_ARRAY(n_elements(log_temp_stemplate), nfiles)
...
for i = 0, nfiles - 1 do begin
	...
	READCOL, stemplateFiles[templatesToUse[i]-1], v1,v2, $
		FORMAT = 'D,D', /SILENT


;	READCOL, stemplateFiles[i], v1,v2, FORMAT = 'D,D', /SILENT

;; Rebinning templates logarthmically
;	lamRange_stemplate = CRVAL1 + [0d, CDELT1*(NAXIS1 - 1d)]
	log_rebin, lamRange_stemplate, v2, log_temp_stemplate, $
		velscale=velscale

;; Normalizing templates
	stemplates[*,i] = log_temp_stemplate/median(log_temp_stemplate)
endfor

;; ----------============== Gas templates  ===============---------

gtemplates = ppxf_emission_lines(logLam_stemplate, FWHM_gal)
ngas = (size(gtemplates,/DIMENSIONS))[1]  ;number of gas emission line templates


component = [replicate(0,nfiles), replicate(1,ngas)]
templates = [[stemplates],[gtemplates]]
;moments = [replicate(moments,nfiles), replicate(2,ngas)]
...
dv = (logLam_stemplate[0]-logLam_bin[0])*c ; km/s


; Find the pixels to ignore to avoid being distracted by gas emission
; lines or atmospheric absorbsion line.  
;goodPixels = ppxf_determine_goodpixels(logLam_bin,lamRange_stemplate,vel, z) 
...
print, 'y = ', y[spaxels_in_bin]
	PPXF, templates, bin_log, noise, velscale, start, $
		bin_dynamics_temp, BESTFIT = bestfit, COMPONENT = component, $
