...
def absorption(line_name, lam, spec, unc_lam=None, unc_spec=None, conv_spec=None, 
	...
	try:
		...
		if unc_lam is not None and unc_spec is not None and conv_spec is not None:
			...
			# ax.plot(spectra.lam,np.array(spectra.flam).flatten(), color='y')
			...
			# ax.plot(spectra.lam,np.array(spectra.flam).flatten(), color='g')
		else:
			corr = 1
			corr_var = 0

		spectra = spectrum(lam=lam, lamspec=spec)
		# ax.plot(spectra.lam,np.array(spectra.flam).flatten(), color='b')
		if noise is not None: 
			variance = spectrum(lam=lam, lamspec=noise**2)
			# ax.plot(spectra.lam,np.array(variance.flam).flatten()*100+0.5, color='m')
		else: variance = None
		index_value, index_va, con, b = spectra.irindex(0.71, line_name, varSED=variance, verbose=False)
		# for i in con:
