...
class population(object):
	...
	def __init__(self, ab_lines, uncerts, interp=None, grid_length=40):
		...
		self.age_prob_dist = np.nansum(np.exp(-chi2_m/2),axis=(1,2))
		self.metal_prob_dist = np.nansum(np.exp(-chi2_m/2),axis=(0,2))
		self.alpha_prob_dist = np.nansum(np.exp(-chi2_m/2),axis=(0,1))
